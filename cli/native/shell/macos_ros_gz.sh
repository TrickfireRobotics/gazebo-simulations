...
# Patch cmake files in the conda env so that imported target names in *-targets.cmake
# match what the config-mode find_package calls actually create.
#   - gz-msgs10:     TINYXML2::TINYXML2 → tinyxml2::tinyxml2 (cppzmq config exports tinyxml2::tinyxml2)
#   - gz-transport13: CPPZMQ::CPPZMQ    → cppzmq             (cppzmq config exports just "cppzmq")
_patch_cmake() {
...
# Drop stale cmake caches so patched conda cmake files are re-read on configure.
find "$ROS_GZ_WS/build" -maxdepth 2 -name "CMakeCache.txt" -delete 2>/dev/null || true
...
colcon build --symlink-install --cmake-args \
	...
	"-DCMAKE_PREFIX_PATH=$ENV_PREFIX;$ROS_BASE" \
